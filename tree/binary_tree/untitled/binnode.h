...
public :
    ...
    BinNodePosi(T) insertAsLC(T const&); //作为当前节点的左节点插入新节点
    ...
    //二叉树递归遍历
    template<typename VST> void traverse(BinNodePosi(T) x , VST & visit){
        if(!x) return;
        visit(x->data);
        traverse(this->lChild , visit);
        traverse(this->rChild , visit);
    }
};
